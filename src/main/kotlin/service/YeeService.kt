...
class YeeService(val ip: String) {
...
//            val screenBitmap = service.getScreenCompressed(screenRect!!)
                ...
                if (color == null)
                    continue
//            val color = getFilteredColor(screenBitmap)
...
//            println("${color[0]}, ${color[1]}, ${color[2]}")
    ...
    private fun initDevice() {
        device = YeelightDevice(ip) //40 166
    ...
    public fun isWorking() = keepWorking
...
//    println(abs(hsb1[2] - hsb2[2]))
    ...
    fun getScreenRect(): Rectangle? {
        ...
        return Rectangle(
            widthCrop,
            heightCrop,
            (screenSize.width - 2 * widthCrop),
            (screenSize.height - 2 * heightCrop)
        )
    }
}